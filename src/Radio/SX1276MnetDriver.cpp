...
SX1276MnetDriver::SX1276MnetDriver()
    : rfState(RfState_t::RX_HEADER_RECEIVE),
      spiSettings(SPISettings(8000000, MSBFIRST, SPI_MODE0)), msgDataOffset(0),
      messageFifo(nullptr) {
  driverObject = this;
}
...
void SX1276MnetDriver::SetBandwidth(float bandwidth) {
  ...
  SpiWriteRegister(SX127X_REG_RX_BW, CalculateBandwidthRegister(bandwidth));
}
...
void SX1276MnetDriver::SetBitrate(float birate) {
  ...
  uint16_t BrFrac =
      roundf(16 * (((SX127X_CRYSTAL_FREQ * 1000.0) / birate) - BrReg));
...
void SX1276MnetDriver::RestartRx() {
  ...
  SpiWriteRegister(SX127X_REG_FIFO_THRESH, SX127X_TX_START_FIFO_NOT_EMPTY |
                                               (HEADER_LENGTH_IN_BYTES - 1));
  // Restart RX
  SpiWriteRegister(SX127X_REG_RX_CONFIG,
                   SX127X_RESTART_RX_WITHOUT_PLL_LOCK | 0x0f);
  FlushFifo();
  rfState = RfState_t::RX_HEADER_RECEIVE;
}
...
*/
void SX1276MnetDriver::GoToIdle(void) { ChangeOperatingMode(SX127X_STANDBY); }
...
void SX1276MnetDriver::ChangeOperatingMode(uint8_t mode) {
  ...
  // After a mode change, we must wait for the ModeReady bit to be set to 1 in
  // the RegIrqFlags1 register However, for some reason, some mode switch never
  // see this bit going to 1. So we have a switch case to only wait for the
  // appropriate modes.
  switch (mode) {
  case SX127X_STANDBY:
  case SX127X_FSTX:
...
void SX1276MnetDriver::SetBaseConfiguration() {
  ...
  SpiWriteRegister(SX127X_REG_SYNC_CONFIG, SX127X_AUTO_RESTART_RX_MODE_NO_PLL |
                                               SX127X_PREAMBLE_POLARITY_55 |
                                               SX127X_SYNC_ON | 0x02);
  ...
  SpiWriteRegister(SX127X_REG_FIFO_THRESH,
                   SX127X_TX_START_FIFO_LEVEL | (HEADER_LENGTH_IN_BYTES - 1));
...
void SX1276MnetDriver::TransmitFromIsr(MicronetMessage_t &message) {
  ...
  if (message.len < 64) {
    ...
    IsrProcessing(ISR_EVENT_TRANSMIT);
  }
}
...
void SX1276MnetDriver::IsrProcessing(uint32_t flags) {
  ...
  if (flags & ISR_EVENT_TRANSMIT) {
    ...
    if ((irqFlags2 & SX127X_FLAG_PACKET_SENT) ||
        (irqFlags2 & SX127X_FLAG_FIFO_EMPTY)) {
      rfState = RfState_t::RX_HEADER_RECEIVE;
      SpiWriteRegister(SX127X_REG_FIFO_THRESH,
                       SX127X_TX_START_FIFO_NOT_EMPTY |
                           (HEADER_LENGTH_IN_BYTES - 1));
      SpiWriteRegister(SX127X_REG_PAYLOAD_LENGTH_FSK, DEFAULT_PACKET_LENGTH);
      ChangeOperatingMode(SX127X_RX);
    }
    ...
    if (irqFlags2 & SX127X_FLAG_FIFO_LEVEL) {
      if (rfState == RfState_t::RX_HEADER_RECEIVE) {
        ...
        SpiBurstReadRegister(SX127X_REG_FIFO, mnetRxMsg.data,
                             HEADER_LENGTH_IN_BYTES);
        mnetRxMsg.startTime_us =
            isrTime - PREAMBLE_LENGTH_IN_US - HEADER_LENGTH_IN_US;
        ...
        if ((checksum == mnetRxMsg.data[MICRONET_CS_OFFSET]) &&
            (mnetRxMsg.data[MICRONET_LEN_OFFSET_1] ==
             mnetRxMsg.data[MICRONET_LEN_OFFSET_2]) &&
            (mnetRxMsg.data[MICRONET_LEN_OFFSET_1] <
             MICRONET_MAX_MESSAGE_LENGTH - 3) &&
            ((mnetRxMsg.data[MICRONET_LEN_OFFSET_1] + 2) >=
          ...
          // Packet content is invalid : ignore it and restart reception
          // for the next packet
          RestartRx();
        }
      } else if (rfState == RfState_t::RX_PAYLOAD_RECEIVE) {
        while (!(SpiReadRegister(SX127X_REG_IRQ_FLAGS_2) &
                 SX127X_FLAG_FIFO_EMPTY)) {
