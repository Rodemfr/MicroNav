...
MicronetDevice::MicronetDevice(MicronetCodec* micronetCodec):
	latestSignalStrength(0), pingTimeStamp(0)
{
...
void MicronetDevice::RemoveLostDevices()
{
	...
	for (int i = 0; i < networkState.nbDevicesInRange; i++)
	{
		if ((now - networkState.devicesInRange[i].lastCommMs) > DEVICE_LOST_TIME_MS)
		{
			...
			{
				memcpy(&networkState.devicesInRange[i], &networkState.devicesInRange[i + 1], sizeof(MicronetDeviceInfo_t) * networkState.nbDevicesInRange - i - 1);
			}
			networkState.nbDevicesInRange--;
		}
	}
