...
// Minimum time in microseconds for CC1101 to restart its XTAL when exiting
// power-down mode
...
const uint8_t CC1101Driver::PA_TABLE[8] = {0xC0, 0x00, 0x00, 0x00,
                                           0x00, 0x00, 0x00, 0x00};
...
CC1101Driver::CC1101Driver()
    : rfFreq_mHz(869.840),
      spiSettings(SPISettings(4000000, MSBFIRST, SPI_MODE0)), lastCSHigh(0),
      freqEstArrayIndex(0), currentFreqOff(0) {
...
CC1101Driver::~CC1101Driver() {
  ...
  pinMode(RF_SCK_PIN, INPUT);
  pinMode(RF_MOSI_PIN, INPUT);
  pinMode(RF_MISO_PIN, INPUT);
  pinMode(RF_CS0_PIN, INPUT);
 ...
 */
void CC1101Driver::SpiWriteBurstReg(uint8_t addr, uint8_t const *buffer,
                                    uint8_t nbBytes) {
  ChipSelect();
 ...
 */
void CC1101Driver::SpiReadBurstReg(uint8_t addr, uint8_t *buffer,
                                   uint8_t nbBytes) {
  ChipSelect();
 ...
 */
int CC1101Driver::GetRxFifoLevel() { return SpiReadStatus(CC1101_RXBYTES); }

/*
 * Get the current RX FIFO level
 *   RETURN level of RX the FIFO in bytes
 */
int CC1101Driver::GetTxFifoLevel() { return SpiReadStatus(CC1101_TXBYTES); }
 ...
 */
void CC1101Driver::IrqOnTxFifoUnderflow() { SpiWriteReg(CC1101_IOCFG0, 0x05); }

/*
 * Configure CC1101 to trigger GDO0 IRQ when TX FIFO is below FIFO threshold
 */
void CC1101Driver::IrqOnTxFifoThreshold() { SpiWriteReg(CC1101_IOCFG0, 0x42); }

/*
 * Configure CC1101 to trigger GDO0 IRQ when RX FIFO is above FIFO threshold
 * or when packet has been entirely received
 */
void CC1101Driver::IrqOnRxFifoThreshold() { SpiWriteReg(CC1101_IOCFG0, 0x01); }
 ...
 * Flush RX FIFO
 */
void CC1101Driver::FlushRxFifo() { SpiStrobe(CC1101_SFRX); }

/*
 * Flush TX FIFO
 */
void CC1101Driver::FlushTxFifo() { SpiStrobe(CC1101_SFTX); }
 ...
 */
void CC1101Driver::SetSidle(void) { SpiStrobe(CC1101_SIDLE); }
...
void CC1101Driver::SetBaseConfiguration(void) {
  ...
  SpiWriteReg(CC1101_DEVIATN,
              0x47); // Default deviation. Will be overwritten later at init
  SpiWriteReg(CC1101_FREND1,
              0x56); // Front-end : value given by Smart RF studio
  SpiWriteReg(CC1101_MCSM0,
              0x08); // PO_TIMEOUT = 2 -> ~150us for XOSC to stabilize
...
void CC1101Driver::UpdateFreqOffset() {
  ...
  // Read latest frequency offset estimation and store it in the averaging
  // array
